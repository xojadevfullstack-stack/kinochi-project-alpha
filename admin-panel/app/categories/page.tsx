  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-text-primary tracking-tight">Kategoriyalar</h1>
          <p className="text-xs sm:text-sm text-text-secondary mt-1">Janrlar va toifalarni boshqarish</p>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 items-end">
          ...
          <div className="flex gap-2 w-full">
            ...
            {editingId && (
              <button 
                type="button" 
                onClick={() => { setEditingId(null); setForm({name: "", slug: "", is_active: true}); }} 
      ...
      <div className="metric-card rounded-2xl overflow-hidden shadow-xl">
        <div className="p-4 sm:p-5 border-b border-white/5 bg-[#1a0908]/80 flex items-center justify-between">
          <h3 className="font-display font-bold text-base sm:text-lg text-text-primary">Kategoriyalar ro'yxati ({categories.length})</h3>
        </div>
            ...
            <tbody className="divide-y divide-white/5 text-sm">
              {categories.map(c => (
                <tr key={c.id} className="data-table-row">
                  ...
                  <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-text-primary font-medium">{c.name}</td>
                  <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-text-secondary text-xs font-mono">{c.slug}</td>
