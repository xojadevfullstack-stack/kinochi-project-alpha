  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-text-primary">Sahifalar</h1>
      </div>
      
      <div className="metric-card p-6 rounded-xl mb-8">
        <h2 className="text-xl font-semibold mb-4 text-text-primary">{editingId ? "Tahrirlash" : "Yangi Sahifa"}</h2>
        <form onSubmit={handleSubmit} className="flex gap-4 items-end flex-wrap">
          <div>
            <label className="block text-sm font-medium text-text-secondary mb-1">Nomi (Sarlavha)</label>
            <input required type="text" className="w-full bg-surface-container-lowest border border-white/10 rounded-lg p-2.5 text-text-primary focus:ring-2 focus:ring-primary-container focus:border-primary-container" value={form.title} onChange={e => setForm({...form, title: e.target.value})} />
          </div>
          <div>
          ...
          {editingId && (
            <button type="button" onClick={() => { setEditingId(null); setForm({title: "", slug: "", is_active: true}) }} className="bg-white/5 border border-white/10 text-text-primary px-6 py-2.5 rounded-lg hover:bg-white/10 h-[46px] font-medium transition-all">
              Bekor qilish
            </button>
          )}
        </form>
          ...
          <tbody className="divide-y divide-white/5">
            {pages.map(p => (
              <tr key={p.id} className="data-table-row">
                <td className="px-6 py-4 whitespace-nowrap text-text-primary font-medium">{p.id}</td>
                <td className="px-6 py-4 whitespace-nowrap text-text-primary font-medium">{p.title}</td>
                <td className="px-6 py-4 whitespace-nowrap text-text-secondary">{p.slug}</td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`px-3 py-1 inline-flex text-xs leading-5 font-bold rounded-full ${p.is_active ? 'bg-primary-container/20 text-primary-container border border-primary-container/30' : 'bg-surface-container-high text-text-secondary border border-white/10'}`}>
                    {p.is_active ? 'Faol' : 'Nofaol'}
                  </span>
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium flex gap-3">
                  <button onClick={() => handleEdit(p)} className="text-tertiary-fixed hover:text-white transition-colors">Tahrirlash</button>
                  <button onClick={() => handleDelete(p.id)} className="text-primary-container hover:text-red-400 transition-colors">O'chirish</button>
                </td>
              </tr>
            ))}
            {pages.length === 0 && (
