...
export async function generateMetadata({ searchParams }: { searchParams: { q?: string } }): Promise<Metadata> {
  ...
  return {
    ...
    description: `"${query}" bo'yicha topilgan barcha kinolar ro'yxati.`,
  };
}
...
export default async function SearchPage({ searchParams }: { searchParams: { q?: string } }) {
  ...
  if (query.length >= 2) {
    try {
      ...
      const response = await fetchApi(`/movies/search?q=${encodeURIComponent(query)}&limit=40`);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-12 max-w-xl">
          <form action="/search" method="GET" className="relative group">
            ...
            <input 
              ...
              placeholder="Boshqa narsa qidirish..." 
            />
        ...
        {errorMsg ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 md:gap-6">
            {movies.map(movie => (
              <Link href={`/movie/${movie.code}`} key={movie.code} className="group relative rounded-xl overflow-hidden bg-surface transition-transform duration-300 hover:scale-105 hover:z-10 ring-1 ring-white/10 hover:ring-primary/50 flex flex-col">
          ...
          query.length >= 2 && (
            <div className="text-center py-20 bg-surface rounded-2xl border border-white/5">
              ...
              <p className="text-gray-400">"{query}" bo'yicha hech qanday kino yoki serial mavjud emas.</p>
            </div>
          )
        )}
      </div>
