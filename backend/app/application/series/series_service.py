...
class SeriesService:
    ...
    async def get_series_by_code(self, code: str) -> Series | None:
        series_model = await self.repository.get_series_by_code(code)
        if not series_model:
            return None
        return Series.model_validate(series_model)
    ...
    async def link_episode_video_from_message(self, episode_id: int, message_id: int, language: str = "Asosiy", source_url: str | None = None, telegram_file_id: str | None = None) -> Episode | None:
        episode = await self.repository.get_episode_by_id(episode_id)
        if not episode:
            return None
